...
namespace dotnet_anime_list.API.Services
{
    public class UserService(IUserRepository repository, AuthService service)
    {
        ...
        {
            var login = await _repository.Login(user.Email, user.Password, ct);
            
            if(login == null)
                throw new Exception("Email or password incorrect");

            var token = await _service.CreateToken(login, ct);
       ...
       {
           var user = await _repository.GetUser(token.UserId, ct);
            if (user == null)
        ...
        {
            var userToUpdate = await _repository.GetUser(token.UserId, ct);
