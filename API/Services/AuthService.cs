...
namespace dotnet_anime_list.API.Services
{
    public class AuthService(ITokenRepository repository, TokenService service)
    {
        ...
        { 
            ...
            return !valid ? throw new Exception("Token expired, please log in again ") : token;
        }
        ...
        {
            var token = await _repository.GetTokenByID(user.Id, ct) ?? throw new Exception("Token not found");
            await _repository.Delete(token, ct);
        }
